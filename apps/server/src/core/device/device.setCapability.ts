import { DcCreationAttributes, DeviceCapabilityAttributes } from '@friday/shared';
...
export default async function setCapability(
  this: DeviceClass,
  deviceId: string,
  capability: DcCreationAttributes
  ...
  const deviceCapabilityObject = await DeviceCapability.create(capability);
