...
describe('Device.openclose', () => {
  it('should set open door', async () => {
    ...
    global.FRIDAY.event.emit(DevicesActionsType.OPEN, {
    ...
    expect(listener.args[0][0].message).to.equal(
      '{"device":"SENSOR-10","method":"action.devices.commands.open","params":{"value":true}}',
    );
  });
  ...
  it('should set close door', async () => {
    ...
    global.FRIDAY.event.emit(DevicesActionsType.CLOSE, {
      id: '8e90fc03-2487-4c96-bec0-f2c73c73168a',
      value: false,
    });
    ...
    expect(listener.called).equal(true);
    expect(listener.args[0][0].message).to.equal(
      '{"device":"SENSOR-10","method":"action.devices.commands.close","params":{"value":false}}',
    );
  });
